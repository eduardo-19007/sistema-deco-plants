...
public class ProductoWebController {
    ...
    public String guardarProducto(
        ...
        // LÓGICA DE SUBIDA Y LIMPIEZA DE IMAGEN
        if (file != null && !file.isEmpty()) {
            try {
                // 1. Si el producto ya existe, borramos su imagen anterior físicamente del disco
                if (producto.getIdProducto() != null) {
                    Producto pExistente = productoService.obtenerPorId(producto.getIdProducto());
                    if (pExistente != null && pExistente.getImagen() != null && !pExistente.getImagen().isEmpty()) {
                        Path rutaImagenVieja = Paths.get("uploads").resolve(pExistente.getImagen());
                        Files.deleteIfExists(rutaImagenVieja);
                    }
                }

                // 2. Guardamos la nueva imagen con su nuevo UUID
            ...
            // Si no subieron ninguna foto nueva, conservamos la que ya tenía en la base de datos
            Producto pExistente = productoService.obtenerPorId(producto.getIdProducto());
            if(pExistente != null){
    ...
    // ELIMINAR PRODUCTO Y SU IMAGEN (GET)
    @GetMapping("/eliminar/{id}")
    public String eliminarProducto(@PathVariable("id") Integer id) {
        // 1. Primero obtenemos el producto para saber el nombre exacto de su imagen
        Producto producto = productoService.obtenerPorId(id);
        
        // 2. Si el producto existe y tiene una imagen válida, la borramos físicamente del disco
        if (producto != null && producto.getImagen() != null && !producto.getImagen().isEmpty()) {
            try {
                Path rutaImagen = Paths.get("uploads").resolve(producto.getImagen());
                Files.deleteIfExists(rutaImagen); // Borra el archivo .jpg/.png de la carpeta
            } catch (Exception e) {
                e.printStackTrace(); // Imprime el error por consola si el archivo está en uso o no se encuentra
            }
        }
        
        // 3. Finalmente, eliminamos el registro de la tabla en SQL Server
